...
### Host Behavior
Only the host can start the swipe session. Host-transfer is not implemented yet — until then, a host cannot leave the party while other active members remain (returns `WatchPartyErrors.HostCannotLeave`). If the host is the last active member, leaving closes the party (`IsActive=false`, `ClosedAt` set).
...
- Host-transfer: should leaving as host transfer ownership to another member, or keep the current behavior (block leave unless last member)?
...
- Email confirmation flow is not implemented; `User.IsEmailConfirmed` defaults to `true` so users can log in immediately after registering.
